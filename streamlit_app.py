...
if not st.session_state.user:
    ...
    if menu == "👑 Admin Panel":
        ...
        # 🌟 แยกแท็บ อนุมัติผู้ใช้เดิม / สร้างผู้ใช้ใหม่
        tab_manage, tab_add = st.tabs(["👥 จัดการคำขอ / อนุมัติ", "➕ สร้างผู้ใช้ใหม่"])
                ...
                if st.form_submit_button("✅ สร้างบัญชี", use_container_width=True):
                    if new_email and len(new_password) >= 6:
                        try:
                            ...
                            if res.user:
                                # อัปเดต Profile ให้ Approved และกำหนด Role อัตโนมัติ
                                ...
                                st.warning("⚠️ **ข้อควรระวัง:** ระบบของฐานข้อมูลอาจทำการล็อกอินบัญชีใหม่ให้อัตโนมัติ หากหน้าเว็บรีเฟรชแล้วคุณกลายเป็นชื่อผู้ใช้ใหม่ กรุณากดปุ่ม 'ออกจากระบบ' ด้านซ้ายมือ แล้วล็อกอินบัญชี Admin ของคุณกลับเข้ามาอีกครั้งครับ")
                                time.sleep(4)
                                st.rerun()
                        except Exception as e:
