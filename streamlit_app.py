...
if not st.session_state.user:
    ...
    # ----------------------------------------------------------------------
    # 📜 ประวัติรับ-จ่าย (V7 - เพิ่มการกรองรายเดือน)
    # ----------------------------------------------------------------------
        ...
        with tab_history:
            ...
            if not df_trans.empty:
                ...
                if selected_ym != "ทั้งหมด":
                    df_display = df_display[df_display['ym'] == selected_ym]
        ...
        with tab_stockcard:
            ...
            if not meds.empty:
                ...
                if not df_t.empty:
                    ...
                    # 1. จัดเรียงจากเก่าไปใหม่ เพื่อให้คำนวณยอดยกไปถูกต้อง
                    df_t = df_t.sort_values(by='created_at', ascending=True)
                    df_t['running_balance'] = df_t['qty_change'].cumsum()
                    
                    # 2. จัดเรียงใหม่จากล่าสุดไปเก่า (เพื่อให้คนอ่านง่าย)
                    ...
                    # 🌟 ตัวกรองแยกรายเดือนสำหรับ Stock Card 🌟
                    ...
                    # ประยุกต์ใช้ตัวกรอง
                    if selected_ym_sc != "ทั้งหมด":
                        df_show = df_t[df_t['ym'] == selected_ym_sc].copy()
                    else:
                        df_show = df_t.copy()
                    ...
                    if not df_show.empty:
                        ...
                        st.info(f"ไม่พบประวัติการเคลื่อนไหวในเดือนที่เลือก")
                else:
