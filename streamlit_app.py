...
if not st.session_state.user:
    ...
    # ----------------------------------------------------------------------
    # 📊 สรุปยอดประจำเดือน และ รายงานขอเบิก (V32 - ระบบเพิ่มลบและแก้ไขตาราง)
    # ----------------------------------------------------------------------
        ...
        # ==========================================
        # 🌟 แท็บที่ 2: รายงานขอเบิก (V32 - แก้ไขและเพิ่ม/ลบได้)
        # ==========================================
        with tab_reorder:
            ...
            if not meds.empty:
                ...
                # หาตัวที่ต่ำกว่าจุดสั่งซื้อ เพื่อนำมาใส่เป็นค่า Default ในกล่องเลือก
                low_stock_df = df_all[df_all['qty'] <= df_all['min_stock']]
                default_selected = low_stock_df['generic_name'].tolist()

                # --- ส่วนที่ 1: ให้เลือกรายการเวชภัณฑ์ ---
                st.markdown("##### 1. ค้นหาและเลือกรายการที่ต้องการขอเบิก")
                st.info("💡 **ทริค:** ระบบเลือกรายการที่ 'ต่ำกว่าจุดสั่งซื้อ' มาให้แล้วอัตโนมัติ คุณสามารถคลิกที่ช่องว่างด้านล่างเพื่อพิมพ์เพิ่มรายการอื่นได้ หรือกดกากบาทเพื่อลบออกครับ")
                
                selected_items = st.multiselect(
                    "ระบุรายการเวชภัณฑ์สำหรับใบเบิก:",
                    options=df_all['generic_name'].tolist(),
                    default=default_selected
                )

                if selected_items:
                    st.markdown("##### 2. แก้ไขจำนวนและดาวน์โหลดใบขอเบิก")
                    st.caption("✏️ คุณสามารถ **ใช้เมาส์คลิกที่ตัวเลขในช่อง 'จำนวนขอเบิก'** บนตารางด้านล่าง เพื่อพิมพ์แก้ไขตัวเลขตามความต้องการได้เลยครับ (ระบบตั้งค่าเริ่มต้นให้เบิกเท่ากับอัตราใช้ต่อเดือน)")
                    
                    # กรองเฉพาะรายการที่ถูกเลือกจากกล่องด้านบน
                    df_selected = df_all[df_all['generic_name'].isin(selected_items)].copy()
                    
                    # สร้างคอลัมน์ แนะนำให้เบิก (ตั้งต้นเท่ากับ min_stock)
                    df_selected['suggested_reorder'] = df_selected['min_stock']

                    # เลือกและจัดเรียงคอลัมน์
                    df_display_reorder = df_selected[['generic_name', 'unit', 'min_stock', 'qty', 'suggested_reorder']].copy()
                    ...
                    # 🌟 ใช้ data_editor เพื่อให้ผู้ใช้กดพิมพ์แก้ตัวเลขบนหน้าเว็บได้เลย
                    edited_df = st.data_editor(
                        df_display_reorder,
                        hide_index=True,
                        use_container_width=True,
                        disabled=["ลำดับ", "รายการ", "หน่วยนับ", "อัตราใช้ต่อเดือน", "จำนวนคงเหลือ"] # ล็อกคอลัมน์อื่นๆ ไม่ให้แก้ ยกเว้นช่องสุดท้าย
                    )

                    st.divider()
                    
                    # ดาวน์โหลดไฟล์ Excel โดยใช้ DataFrame ที่ผ่านการแก้ไข (edited_df)
                    buffer = io.BytesIO()
                    try:
                        edited_df.to_excel(buffer, index=False, sheet_name='ใบขอเบิก')
                        st.download_button(
                        ...
                        csv_reorder = edited_df.to_csv(index=False).encode('utf-8-sig')
                        st.download_button(
                    ...
                    st.success("✅ ยอดคงคลังเพียงพอทุกรายการ (หากต้องการเบิกเพิ่ม ให้คลิกพิมพ์ที่กล่องค้นหาด้านบนได้เลยครับ)")
            else:
                st.warning("ไม่พบข้อมูลเวชภัณฑ์ในระบบ")
