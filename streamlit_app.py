...
def login_user(email, password):
    try:
        ...
        else: st.error("❌ ไม่พบข้อมูลสิทธิ์ผู้ใช้ หรือบัญชีถูกลบไปแล้ว")
...
if not st.session_state.user:
    ...
    with col2:
        ...
        with tab_register:
            with st.form("register_form"):
                st.info("💡 สมัครสมาชิกใหม่ แล้วรอ Admin กดอนุมัติเพื่อเข้าใช้งาน")
                ...
                if st.form_submit_button("สมัครสมาชิก", use_container_width=True):
                    if reg_email and len(reg_password) >= 6:
                        try:
                            ...
                            st.success("✅ สมัครสมาชิกสำเร็จ! โปรดแจ้ง Admin ให้กดอนุมัติ")
                        except Exception as e:
    ...
    if menu == "👑 Admin Panel":
        ...
        with tab_add:
            ...
            st.info("💡 บัญชีที่สร้างจากหน้านี้ จะได้รับการอนุมัติ (Approved) และพร้อมใช้งานทันที")
                    ...
                    if new_email and len(new_password) >= 6:
                        try:
                            ...
                            if res.user:
                                ...
                                st.warning("⚠️ **ข้อควรระวัง:** หลังจากนี้ให้กดปุ่ม 'ออกจากระบบ' แล้วล็อกอินบัญชี Admin กลับเข้ามาอีกครั้งครับ")
                                time.sleep(4)
                                st.rerun()
                        except Exception as e:
                            st.error(f"❌ ไม่สามารถสร้างบัญชีได้: {e}")
                    else:
                        st.warning("⚠️ กรุณากรอกอีเมล และรหัสผ่านขั้นต่ำ 6 ตัวอักษร")

        with tab_delete:
            st.subheader("🗑️ เพิกถอนสิทธิ์ / ลบบัญชีผู้ใช้งาน")
            st.warning("⚠️ การลบผู้ใช้งานออกจากตารางนี้ จะทำให้ผู้ใช้คนนั้น **ไม่สามารถเข้าสู่ระบบแอปพลิเคชันได้อีก** (ประวัติการคีย์ข้อมูลของเขาจะยังคงอยู่)")
                ...
                if not other_users.empty:
                    ...
                    if st.button("❌ ลบผู้ใช้งาน", type="primary"):
                        if confirm_del_user:
                            ...
                            st.error("กรุณาติ๊กช่องยืนยันก่อนกดปุ่มลบครับ")
                else:
                    st.info("ไม่มีผู้ใช้งานอื่นในระบบให้ลบครับ")

    # ----------------------------------------------------------------------
    # 📊 แดชบอร์ด (V17 - ปรับปรุงตามมาตรฐานเภสัชกรรม)
    # ----------------------------------------------------------------------
    elif menu == "📊 แดชบอร์ด":
        st.header("📊 ภาพรวมคลังเวชภัณฑ์ (Dashboard)")
        try:
            ...
            if not meds.empty:
                ...
                if not inv.empty:
                    ...
                    if not inv_active.empty:
                        ...
                        # 🌟 ปรับเกณฑ์ใกล้หมดอายุจาก 180 วัน เป็น 90 วัน (3 เดือน)
                ...
                with col_r:
                    ...
                    st.caption("รายการที่จะหมดอายุภายใน 3 เดือนข้างหน้า (90 วัน) - ต้องเร่งกระจายตามหลัก FEFO")
            ...
            if all_months:
                ...
                if not meds.empty:
                    ...
                    st.download_button(
                        ...
                        data=csv,
                        file_name=f'สรุปคลังยา_{selected_ym}.csv',
                        mime='text/csv',
                        type="primary"
                    )
                else:
                    st.warning("ไม่พบข้อมูลยาในระบบ")
            else:
